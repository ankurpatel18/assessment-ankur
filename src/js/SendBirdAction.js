...
class SendBirdAction {
  ...
  getGroupChannelList(isInit = false) {
    if (isInit || isNull(this.groupChannelQuery)) {
      ...
      console.log('this.groupChannelQuery ', this.groupChannelQuery);
      this.groupChannelQuery.channelUrlsFilter = ['AnkurGroup', 'bonus4'];
    }
    return new Promise((resolve, reject) => {
  ...
  createGroupChannel(userIds) {
    return new Promise((resolve, reject) => {
      ...
      params.addUserIds(userIds);
      params.operatorIds = ['au1'];   // Or .operators(Array<User>)
      params.name = "Ankur";
      params.channelUrl = "AnkurGroup"; // In a group channel, you can create a new channel by specifying its unique channel URL in a 'GroupChannelParams' object. 
      params.customType = "Ankur";
